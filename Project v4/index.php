    ...
    <main>
        ...
        <section class="posts">
            ...
            <?php while($row = $statement->fetch()): ?>
                <div class="coffee-shop-card" onclick="expandCard(this)">
                    <div class="coffee-shop">
                        <h3><?= $row['Name'] ?></h3>
                        <p class="edit"><a href="update.php?id=<?= $row['Shop_id'] ?>"> - Edit</a></p>
                        <p><?= $row['Hours'] ?></p>
                        <p><a href="<?= $row['Website'] ?>" target="_blank"><?= $row['Website'] ?></a></p>
                        <p><?= substr($row['Description'], 0, 50) . (strlen($row['Description']) > 50 ? "..." : "") ?></p>
                    </div>
                </div>
            <?php endwhile; ?>
        </section>
